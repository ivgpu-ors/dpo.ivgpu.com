    ...
    <section class="grid lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16">
      @foreach($courses as $course)
        <article class="border border-gray-400 rounded shadow-md flex flex-col justify-between overflow-hidden">
          <header class="p-2">
            ...
            @if($course->start)
              <div class="text-gray-500 mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22" class="h-7 inline">
                  <path
          ...
          <footer>
            @if($course->enabled)
              <a href="" class="bg-primary text-light w-full h-full block p-2 text-center hover:bg-primary-dark">Идет прием заявок</a>
            @else
              <a href="" class="bg-red-400 hover:bg-red-500 text-light w-full h-full block p-2 text-center">Прием заявок завершен</a>
            @endif
          </footer>
        </article>
      @endforeach
